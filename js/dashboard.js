...
async function loadStats() {
    ...
    const [{ count: wins }, { count: losses }, { count: friendCount }] = await Promise.all([
    ...
    const { count: total } = await sb.from('games').select('*', { count: 'exact', head: true })
        .or(`player_white.eq.${uid},player_black.eq.${uid}`).eq('status', 'finished');

    setEl('statWins', wins || 0);
    setEl('statLosses', losses || 0);
    setEl('statDraws', (total || 0) - (wins || 0) - (losses || 0));
    setEl('statFriends', friendCount || 0);
}
...
async function loadRecentGames() {
    ...
    list.innerHTML = data.map(g => {
        ...
        if (g.winner) {
            const wonAsWhite = g.winner === 'white' && isWhite;
            const wonAsBlack = g.winner === 'black' && !isWhite;
            result = (wonAsWhite || wonAsBlack) ? 'win' : 'loss';
            resultLabel = result === 'win' ? 'Win' : 'Loss';
        }
